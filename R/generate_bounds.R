...
#' event <- tribble(~H1, ~H2, ~Analysis, ~Event,
#'                  1, 1, 1, 155,
#'                  2, 2, 1, 160,
#'                  3, 3, 1, 165,
#'                  1, 2, 1, 85,
...
generate_bounds <- function(type = 1, k = 2, w = w, m = m, 
  ...
  for (j in 1:nrow(inter_weight)) {
    ...
    if (type == 0) { # Bonferroni
      ...
      bounds_tmp <- tibble::tibble(Analysis = 1:k, 
      ...
      for (i in 1:k){
        ...
        if (type == 3) {
          ...
          for (h in hypotheses) {
            ...
            p_B_tmp <- 1 - pnorm(gsDesign::gsDesign(k = k, 
                                                    test.type = 1, 
                                                    usTime = t[[h]], 
                                                    n.I = corr_B_tmp[,ncol(corr_B_tmp)]^2,
                                                    alpha = alpha * w_tmp0[h], 
                                                    sfu = sf[[h]], 
                                                    sfupar = sfparm[[h]])$upper$bound)[i]
